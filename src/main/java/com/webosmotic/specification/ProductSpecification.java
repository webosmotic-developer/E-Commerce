		...
		return (Specification<Product>) (root, query, cb) -> {
			...
			if (searchCriteria != null) {
				...
				if (!Strings.isNullOrEmpty(searchCriteria.getAll())) {
					final Predicate namePredicate = cb.like(cb.lower(root.get("name")),
							createPattern(searchCriteria.getAll()));
					final Predicate descriptionPredicate = cb.like(cb.lower(root.get("description")),
							createPattern(searchCriteria.getAll()));
					final Predicate criteriaPredicate = cb.like(
							cb.lower(root.get("productCategory").get("parentCategory")),
							createPattern(searchCriteria.getAll()));
					final Predicate subCriteriaPredicate = cb.like(
							cb.lower(root.get("productCategory").get("subCategory")),
							createPattern(searchCriteria.getAll()));
					final Predicate brandPredicate = cb.like(cb.lower(root.get("brand")),
							createPattern(searchCriteria.getAll()));
					final Predicate allPredicate = cb.or(namePredicate, descriptionPredicate, criteriaPredicate,
							subCriteriaPredicate, brandPredicate);
				...
				if (!Strings.isNullOrEmpty(searchCriteria.getDescription())) {
					predicates.add(
							cb.like(cb.lower(root.get("description")), createPattern(searchCriteria.getDescription())));
				}
				...
				if (!Strings.isNullOrEmpty(searchCriteria.getBrand())) {
					predicates.add(cb.like(cb.lower(root.get("brand")), createPattern(searchCriteria.getBrand())));
				}
				...
				if (searchCriteria.getMinPrice() > 0 && searchCriteria.getMaxPrice() > 0) {
					predicates.add(cb.between(root.get("unitPrice"), searchCriteria.getMinPrice(),
							searchCriteria.getMaxPrice()));
				}

				if (searchCriteria.getMinDiscount() > 0 && searchCriteria.getMaxDiscount() > 0) {
					predicates.add(cb.between(root.get("discount"), searchCriteria.getMinDiscount(),
							searchCriteria.getMaxDiscount()));
				}
			}
