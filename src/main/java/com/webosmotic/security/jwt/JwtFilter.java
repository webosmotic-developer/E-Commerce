...
/**
 * Custom Filter class to tap every incoming request once to check if the
 * request contains the jwt token. If contains JWT token then validate the token
 * and pass the request If not then pass the request.
 */
...
public class JwtFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (jwt != null && jwtProvider.validate(jwt)) {
			try {
				final String userName = jwtProvider.getUserName(jwt);
